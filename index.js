/**
 * 🦞 Project Golem v6.4 (Ouroboros Ultimate)
 * ---------------------------------------------------
 * 架構：[Gemini 大腦] -> [Ollama 翻譯官] -> [Security 審計官] -> [Node.js 執行者]
 * 特性：自我內省、熱修復補丁 (Fuzzy Match)、自主進化 (Persistent)、人格記憶、冒煙測試
 */
...
const { exec, execSync, spawn } = require('child_process'); // ✨ 新增 spawn
...
// ============================================================
// 🧠 Experience Memory (經驗記憶體) [Batch 1: 持久化排程]
// ============================================================
...
// ============================================================
// 🩹 Patch Manager (神經補丁管理) [Batch 2 & 3: 模糊匹配與沙箱]
// ============================================================
class PatchManager {
  ...
  static verify(filePath) {
    try {
      ...
      // ✨ 階段 2: 運行時冒煙測試 (Runtime Smoke Test)
      // 使用 GOLEM_TEST_MODE=true 環境變數執行，設定 5 秒超時
      ...
      console.log("✅ [PatchManager] 冒煙測試通過 (Smoke Test Passed)");
      return true;
    } catch (e) {
      console.error(`❌ [PatchManager] 驗證失敗: ${e.message}`);
      if (e.stderr) console.error(`錯誤詳情: ${e.stderr.toString()}`);
      return false;
    }
  }
}
...
// ============================================================
// 🛡️ Security Manager (安全審計) [Batch 3: 強化規則]
// ============================================================
class SecurityManager {
  constructor() {
    ...
    this.BLOCK_PATTERNS = [
      /rm\s+-rf\s+\//,             // 禁止根目錄刪除
      /rd\s+\/s\s+\/q\s+[c-zC-Z]:\\$/, // 禁止 Windows 磁碟刪除
      />\s*\/dev\/sd/,             // 禁止寫入原始設備
      /:(){:|:&};:/,               // 禁止 Fork Bomb
      /mkfs/, /Format-Volume/,     // 禁止格式化
      /dd\s+if=/,                  // 禁止 dd 破壞
      /chmod\s+[-]x\s+/            // 禁止移除執行權限
    ];
  }
...
// ============================================================
// 📖 Help Manager (動態說明書) [Batch 4: 內省功能列表]
// ============================================================
class HelpManager {
  static getManual() {
    // 1. 讀取自身原始碼
    ...
    // 2. 正則掃描：抓取所有 /開頭的指令
    const cmdPattern = /text\.(?:startsWith|includes)\(['"](\/[a-zA-Z0-9_]+)['"]\)/g;
    const foundCmds = new Set();
    let match;
    while ((match = cmdPattern.exec(source)) !== null) {
      foundCmds.add(match[1]);
    }

    // 3. 掃描 Skills
    let skillList = "基礎 Shell 操作";
    try {
        skillList = Object.keys(skills).filter(k => k !== 'persona' && k !== 'getSystemPrompt').join(', ') || "基礎 Shell 操作";
    } catch(e) {}
...
💡 **虛擬指令:**
• \`golem-help\` : 呼叫本選單 (由 Gemini 自動觸發)

*提示：您可以直接對我說「幫我更新代碼」或「檢查系統狀態」，我會自動規劃指令。*
`;
  }
}

// ============================================================
// 🕰️ Autonomy Manager (自主進化排程) [Batch 1: 持久化與恢復]
// ============================================================
class AutonomyManager {
  ...
  start() {
    ...
    console.log("🕰️ [Autonomy] 自主進化模組檢查中...");
    
    // ✨ 檢查是否有未完成的等待
    const now = Date.now();
    if (memory.data.nextWakeup > now) {
      const waitMs = memory.data.nextWakeup - now;
      console.log(`♻️ 恢復排程：繼續休眠 ${(waitMs / 3600000).toFixed(2)} 小時`);
  ...
  scheduleNextAwakening() {
    ...
    const waitMs = nextWaitHours * 60 * 60 * 1000;

    // ✨ 保存下次喚醒時間
    memory.data.nextWakeup = Date.now() + waitMs;
    memory.save();

    console.log(`💤 [Autonomy] 進入休眠，預計 ${nextWaitHours.toFixed(1)} 小時後 (${new Date(memory.data.nextWakeup).toLocaleString()}) 醒來。`);
...
// ============================================================
// 🔍 System Fingerprint (環境感知) [Batch 4: 注入虛擬指令提示]
// ============================================================
...
- 建議指令集: ${shellType}
- 特殊能力: 若使用者詢問功能列表或幫助，請在 Action Plan 中生成 "run golem-help" 指令。
- 工作目錄: ${process.cwd()}
`;
  } catch (e) {
...
// ============================================================
// ⚡ Task Controller (核心控制) [Batch 4: 虛擬指令攔截]
// ============================================================
class TaskController {
  ...
  async runSequence(chatId, steps, startIndex = 0) {
    ...
    for (let i = startIndex; i < steps.length; i++) {
      ...
      // ✨ 攔截虛擬指令：說明書請求
      if (step.cmd.trim() === 'golem-help') {
        const manual = HelpManager.getManual();
        await bot.sendMessage(chatId, manual, { parse_mode: 'Markdown' });
        logBuffer += `✅ [System] 已顯示功能列表\n`;
        continue; // 跳過 Shell 執行，繼續下一個步驟
      }
...
// 🛡️ 安全入口：支援測試模式 (防止測試時重複登入/啟動) [Batch 3: 測試入口]
if (process.env.GOLEM_TEST_MODE === 'true') {
  console.log("🧪 [TestMode] 模組載入與語法檢查通過。");
  process.exit(0);
}
...
async function executeDeploy(chatId) {
  ...
  try {
    ...
    // ✨ 🔄 Ouroboros Respawn: 啟動新進程並分離 [Batch 1]
...
bot.on('message', async (msg) => {
  ...
  // 🌟 0. 優先：求助指令 [Batch 4: 自然語言轉 Help]
  if (text === '/help' || text === '/menu' || text === '指令' || text === '功能') {
    const manual = HelpManager.getManual();
    await bot.sendMessage(chatId, manual, { parse_mode: 'Markdown' });
    return;
  }
...
console.log('📡 Golem v6.4 (Ouroboros Ultimate) is Online.');
